...
class HybridFPN(nn.Module):
    def __init__(self, num_classes, use_hyperbolic_encoder=True, use_hyperbolic_decoder=False):
        super().__init__()
        self.using_hyperbolic_encoder = use_hyperbolic_encoder
        self.using_hyperbolic_decoder = use_hyperbolic_decoder

        if use_hyperbolic_encoder:
            self.manifold = CustomLorentz(k=1.0, learnable=False)
            self.backbone = Lorentz_resnet18(manifold=self.manifold)
            self.feature_channels = [65, 129, 257, 513]
            self.fpn_channels = 257
    ...
    def decode_hyperbolic(self, c1, c2, c3, c4):
        # Build FPN
        # Top-down pathway
        p4 = self.lateral4(c4)
        p3 = self.lateral3(c3) + self.upsample_bhwc(p4)
    ...
    def decode_euclidian(self, c1, c2, c3, c4):
        # Build FPN
        # Top-down pathway
        p4 = self.lateral4(c4)
        p3 = self.lateral3(c3) + F.interpolate(p4, scale_factor=2, mode='nearest')
    ...
    def forward(self, x):
        if self.using_hyperbolic_encoder:
            features = self.backbone(x, max_pool=True, return_features=True)
        else:
            features = []
            for i, module in enumerate(self.backbone):
                x = module(x)
        ...
        if self.using_hyperbolic_decoder:
            # Do projection if coming from euclidian
            return self.decode_hyperbolic(*features)
        else:
            # Do something when coming from hyperbolic
            return self.decode_euclidian(*features)