...
// Tile dimensions for the spatial x- and y-axes handled by a thread-block.
#define TILE_DIM 8

// Number of channel groups computed in parallel by different z-threads.
// A larger value schedules more threads per block (TILE_DIM*TILE_DIM*CHANNEL_GROUPS)
// but reduces work per thread in the innermost channel-loop. Tune as needed.
#define CHANNEL_GROUPS 4
...
__global__ void fused_lorentz_conv2d_kernel_tiled(
    ...
    const int tz = threadIdx.z;            // channel-group index inside the block

    // Unique linear thread id across (x, y, z) – used for cooperative loads.
    const int thread_idx_flat = (tz * TILE_DIM * TILE_DIM) + (ty * TILE_DIM + tx);

    // Total number of threads participating in cooperative loads.
    const int threads_per_block = blockDim.z * TILE_DIM * TILE_DIM;
    ...
    // ---------------------------------------------------------------------
    // Static shared memory for partial space sums and the time component.
    // 64 floats * 2  ≈ 512 B – negligible compared to dynamic allocation.
    __shared__ float s_space_accum[TILE_DIM * TILE_DIM];
    __shared__ float s_time_val[TILE_DIM * TILE_DIM];

    // --- Cooperative Loading of Weights ---
    const int weight_row_start = out_ch * weight_features;
    for (int i = thread_idx_flat; i < weight_features; i += threads_per_block) {
    ...
    for (int i = thread_idx_flat; i < tile_size_elements; i += threads_per_block) {
        int c = i % in_channels;
    ...
    __syncthreads(); // Wait for input tile to be loaded and shared arrays ready

    // Initialise shared accumulators once per (tx,ty) position.
    if (tz == 0) {
        s_space_accum[ty * TILE_DIM + tx] = 0.0f;
        s_time_val[ty * TILE_DIM + tx]   = 0.0f;
    }
    ...
    if (out_y < out_height && out_x < out_width) {
        float time_sum_sq   = 0.0f; // only meaningful for tz==0
        float space_partial = 0.0f; // partial contribution of this tz thread
        ...
        for (int ky = 0; ky < kernel_h; ky++) {
            for (int kx = 0; kx < kernel_w; kx++) {
                ...
                // Iterate over channel subset handled by this tz thread.
                for (int ch = 1 + tz; ch < in_channels; ch += blockDim.z) {
                    float space_val = s_input[s_idx_base + ch];
                    int   w_idx     = (ky * kernel_w + kx) * (in_channels - 1) + (ch - 1) + 1; // same original layout
                    space_partial   += space_val * s_weights[w_idx];
                }
            }
        }

        // Store partial space sum into shared memory.
        atomicAdd(&s_space_accum[ty * TILE_DIM + tx], space_partial);

        // Compute time component only once (tz==0)
        if (tz == 0) {
            float aggregated_time = sqrtf(fmaxf(0.0f, time_sum_sq - (float)(kernel_h * kernel_w - 1) * k_value));
            s_time_val[ty * TILE_DIM + tx] = aggregated_time * s_weights[0];
        }
    }

    // Ensure all partial sums and time components are written before final write.
    __syncthreads();

    if (out_y < out_height && out_x < out_width && tz == 0) {
        float space_result = s_space_accum[ty * TILE_DIM + tx];
        float final_result = s_time_val[ty * TILE_DIM + tx] + space_result;
...
torch::Tensor fused_lorentz_conv2d_cuda(
    ...
    dim3 block_dim(TILE_DIM, TILE_DIM, CHANNEL_GROUPS);
    dim3 grid_dim(
